...
抗力そのものについては無条件安定になる。ただし **`α_max` を上げると別の要因で
発散する設定がある**（issue #21）。ある配置では `α_max = 10⁷`（陽解法の限界
`2/dt = 10⁴` の 1000 倍）まで 2000 ステップ安定に回るが、粒子ブロックの形を
変えると `α_max = 10⁵` で NaN になる。原因は未特定なので、
**「無条件安定になった」とは書けない**。α を上げるときは各設定で確認すること。
...
- [ ] `α_max` を上げたときに一部の設定で発散する原因の特定（#21）
- [ ] チェックポイント化して長時間の逆伝播へ
