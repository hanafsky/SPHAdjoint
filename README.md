...
4. ~~**`gtheta` への浮動小数アトミック**~~ → Apple8 / Metal 4 でそのまま動く。
   CPU 随伴と 1.2e-06 で一致。
5. ~~**`04_interactive.jl` の Makie API**~~ → 動いた。tick イベント内での
   シミュレーション実行・Observable 更新・マウス塗りとも問題なし。直したのは
   3 点だけ: 日本語フォントの明示（既定フォントに日本語グリフが無く error）、
   陽解法時代の `@assert dt < 2/α_max` の削除（抗力は陰解法化済み）、
   画面外に生成されたウィンドウの位置補正。
...
ハードコードをやめ、**起動時にその場で較正する**（ウォームアップ 50 ステップ →
400 ステップ計測 → 描画分の余裕 2 割を残して `nsub = 0.8·sps/30`、上限は
実時間 1 倍に当たるステップ数）。CPU/Metal・粒子数を変えても合う。

実測（2026-08-02、M2 CPU / Float64、dp=0.016 → N=375, dt=2e-4）:
13,600 step/s → `nsub = 167` で **30〜31 fps・実時間比 ×1.0** が出て、
固体を塗りながら回しても破綻しない。この規模では CPU で足りるので
Metal に切り替える理由は無い（fps と実時間比はウィンドウ右下に常時表示される）。

### `04_interactive.jl` の感度オーバーレイ（#33）

「感度を見る」ボタンで、押した瞬間の状態から 0.6 s（3000 ステップ）だけ
tape 付きで先読みし、`backward!` の ∂J/∂θ を「どこを固体にすると J が
下がるか」（青 = 塗ると良い）として重ねる。一発の計算に 1 秒弱かかるので
リアルタイム更新はしない。実装時に実測で得た注意 3 つ:

- **配線は FD で照合済み**（δ=1e-6 の中心差分と 0.07% 一致）。ただし
  δ ≥ 1e-4 ではもう合わない。0.6 s 先の J は極度に非線形なので、感度は
  「微小変化の向き」として読むこと。1 セル塗り切った結果の予測ではない。
- **水が目標に届かないと J ≈ 0 になり、勾配は指数の裾のノイズになる**
  （`10_design_gradcheck.jl` と同じ罠）。しかも勾配は粒子の通り道にしか
  乗らないので、ノイズでも毎回それらしい分布に見える。|J| < 0.5 のときは
  表示せず警告し、古い分布も消す（残すと「何をしても感度が変わらない」
  ように見える）。
- **既定の近傍リスト `interval=4` はダムブレイク先端（~3 m/s）で変位が
  skin/2 を超える**。04 の tick ループは `check_neighbor_flags` を通らない
  ため静かに劣化していた。03 の `NL_INTERVAL` と同じく `interval=2` にする。
  状態を手で書き換えた直後は「初回扱い」で `build_neighbors!` を強制すること
  （スクリプト内の `teleport!` を参照）。
...
scripts/          探索コード（すべて Literate.jl 形式）
  ...
  10_design_gradcheck.jl  03 の設計変数勾配（連鎖律 + フィルタ随伴）vs 中心差分
test/runtests.jl
...
**Borrvall–Petersson の罰則 `qp`** だった。ただし効く方向は当初の予想の
**逆**で、`qp` を**下げる**と真の固体が出る（次節）。
...
### qp を下げると真の固体が出る（#24 の答え）

issue #24 には「`qp` の continuation（大きい q から始めて下げる…）」ではなく
「q が小さいほど中間密度に厳しい」と書いていたが、**式を見ると逆**：
`α(ρ) = α_max·q(1-ρ)/(q+ρ)` は q が**大きい**ほど線形に近く、中間密度でも
α が大きい（= 中間密度に厳しい）。そして実測すると、**「中間密度に厳しくすれば
0/1 に行く」という FEM トポ最適の直感自体がこの問題では成り立たない**：

| `qp` | 反復 | J（改善率） | Mnd(rd) | min(rd) | ρ<0.1 のセル | mean(rd) |
|---:|---:|---|---:|---:|---:|---:|
| 10 | 37 | -389.36（15.24%） | 0.50% | 0.910 | 0 | 0.9987 |
| 1 | 31 | -386.87（14.50%） | 0.76% | 0.869 | 0 | 0.9979 |
| 0.1（既定） | 99 | -388.29（14.92%） | 2.24% | 0.454 | 0 | 0.9911 |
| **0.01 直行** | 145 | -401.19（18.74%） | 1.49% | **0.0000** | **13** | 0.9702 |
| **0.01 continuation** | 91+48+15 | **-402.30（19.06%）** | 1.25% | **0.0000** | **11** | 0.9747 |
| 0.003（cont. 続き） | +2 | -393.89 | 1.25% | 0.0000 | 11 | 0.9747 |

（Metal 設定, `α_max = 2000`, 反復上限 200。continuation は
q = 0.1 → 0.03 → 0.01 と下げ、前段の解を次段の初期値にする。
`QP` と `RD0` を外から与えて `include` を繰り返すだけで組める）

- **qp を上げると α_max を上げたときと同型の機構で設計が薄くなる**
  （`α(0.75)/α_max` が 0.029 → 0.143 → 0.233 と増え、浅掘りで抗力が足りる）
- **qp = 0.01 で初めて ρ=0 の真の固体が現れる**。中間密度の抗力が安い
  （`α(0.5)/α_max ≈ 1%`）ので、抗力が欲しければ深く掘るしかないため
- **J も 4 ポイント改善**（14.9% → 18.7〜19.1%）。深い固体は流れを強く曲げられる
- 出てきた設計は**右下の隅（目標 (0.85, 0.10) のすぐ先）の階段状の土手**。
  右壁での跳ね返り（nsteps 検討で見た「ピーク以降 J が減る」の原因）を殺して
  水を目標に留める配置で、物理的に説明がつく
- **continuation は必須ではなかった**（直行 -401.2 vs continuation -402.3、差 0.3%）。
  q=0.01 の初期勾配は 1/10 に潰れる（|grad| 260 → 28）が、`STEP0 = 0.05` で
  まだ動ける。さらに 0.003 へ下げても収穫なし（2 反復・0.06%）
- Mnd(rdf) は 2.6% → 3.5% と**上がる**が、これは真の 0/1 構造ができたからこそ
  フィルタ（`filt_r = 2`）が境界に遷移帯を作るため。悪化ではない

Heaviside 射影（tanh + β continuation）は**使わずに済んだ**。既定の `qp` は
0.1 のまま（既存の実測との照合を保つため）。シャープな設計が欲しいときは
`QP = 0.01` を与える。

設計変数側の連鎖律（`gθ → dα/dρ → フィルタ随伴`）は `01_gradcheck.jl` では
検証されないので、`scripts/10_design_gradcheck.jl` を追加した。q = 10 / 0.01 の
両端で中心差分と 4e-7〜1.3e-6 で一致する。**検証点は「水が目標に届く内点」に
置くこと** — 最初 `rd ≈ 0.6` の一様乱数で試したら水が届かず、`J = -1.5e-07` の
裾ノイズどうしを比較して「一致」を出していた（勾配ゼロの点も 2/3 引いた）。
スクリプトは |J| < 1e-3 なら error で止まり、検証点は |grad| 上位成分から選ぶ。
...
- [x] `04_interactive.jl` を実際に動かす（30〜31 fps・実時間比 ×1.0 @ CPU N=375。
      `nsub` は起動時に自動較正）
- [x] 抗力項を陰的にする（+ 随伴の更新と再検証）
- [x] 設計場をシャープにする → **`qp` を下げる**のが答えだった（`QP = 0.01` で
      ρ=0 の真の固体が出る。continuation・Heaviside 射影は不要。#24 と
      「qp スイープ」節を参照）
- [ ] チェックポイント化して長時間の逆伝播へ
- [x] `04_interactive.jl` に「感度を見る」ボタン（#33。0.6 s 先読みの ∂J/∂設計を
      オーバーレイ表示。FD と 0.07% 一致。詳細は「感度オーバーレイ」節）
