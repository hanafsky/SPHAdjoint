...
**Metal 上の非決定性（長時間積分では効いてくる）**: `_cl_fill!` のアトミック
cursor によりセル内の粒子順序が実行ごとに変わるため、総和順序の丸めで
**同一入力でも 1 ステップあたり ~1e-7 揺らぐ**（CPU はビット再現する）。

問題は、これが**カオス的に増幅する**こと。N=1500 を 5333 ステップ（0.8 秒）
積分すると、同一設計に対する目的関数が **0.29% ばらつく**（J ≈ -338 に対して
最大差 0.98）。`03_optimize.jl` の直線探索が終盤に受理していた改善幅は
0.01〜0.1 で、**ノイズ床の 10〜100 分の 1**。つまり後半の受理判定は
意味を成していない（全体の改善 25.8 はノイズ床より十分大きいので、
最適化そのものは効いている）。

根治するならセル内の粒子順序を決定的にすればよい（counting sort のあとで
各セルの区間を粒子 id でソートする。セルあたり数十粒子なので挿入ソートで足りる）。
当座の回避策としては、直線探索の受理閾値をノイズ床より大きく取るか、
直線探索だけ CPU で回す。
...
|---:|---:|---:|---:|
| 925 | 3243 | 12609 | 3.89x |
| 3750 | 1202 | 11530 | 9.59x |
| 15000 | 593 | 6032 | 10.17x |
| 60000 | 202 | 2175 | 10.79x |

（Metal は実行ごとに ±10% 程度揺れる。上は代表値。**測定前に必ずウォームアップ
すること** — Metal カーネルの初回コンパイルを含めると 1/10 の数字が出る。）
...
セル幅 2h の 3×3 走査は候補 61 個を舐めて実近傍は 21 個（π/9 ≈ 35%）で、
残り 65% が空振り。しかも物理カーネルは前進 2 本 + 随伴 2 本の 4 本あるので、
この無駄が 4 回繰り返される。

**Metal は 1.4〜1.6 倍速くなったが、CPU は 15〜41% 遅くなった**（同一ハーネスで
変更前のコミットと A/B 測定）:

| N | CPU 前 → 後 | Metal 前 → 後 |
|---:|---|---|
| 925 | 3805 → 3243 (0.85x) | 9146 → 12609 (1.38x) |
| 3750 | 2033 → 1202 (0.59x) | 7342 → 11530 (1.57x) |
| 15000 | 853 → 593 (0.70x) | 3829 → 6032 (1.58x) |
| 60000 | 266 → 202 (0.76x) | 1487 → 2175 (1.46x) |

転置レイアウトは GPU のコアレスには最適だが **CPU には最悪**で、同じ粒子の
隣接する近傍が `N·4` バイト離れるためキャッシュラインを毎回外す。
バックエンドごとにレイアウトを変えるのが筋（issue #16）。

**レイアウトが全てだった。** 素直な CSR（offsets + 可変長 indices）は構築が
1438 μs (N=60000) とセルリスト 138 μs の 10 倍かかって割に合わない
（density はむしろ 0.79x に悪化、1 ステップ総計で 1.05x）。行長を固定して
**転置レイアウト**（粒子 i の m 番目の近傍を `indices[(m-1)*N+i]` に置く）に
変えると：
...
同じ m で全スレッドが連続アドレスを触るので読み書きがコアレスし、行長が既知
なので count パス・cumsum・offsets が丸ごと不要になる（2 パス版はこの前段だけで
419 μs 使っていた）。

### 近傍リストの再利用と、その安全条件

物理カーネルが `r² < (2h)²` で絞るので、リストが真の近傍の**上位集合**で
ありさえすれば、毎ステップ構築した場合と**厳密に同じ相互作用集合**になる。
カットオフを `2h(1+skin)` で作れば、各粒子の変位が `skin·h` 以下の間これが保たれる。

ここから、**前進と随伴で再構築のタイミングを揃える必要が無い**ことも従う
（相互作用集合が同じなら計算する関数が同じなので離散随伴は厳密なまま）。
`test/runtests.jl` に「`interval=1` と `interval=8` で実効的な相互作用集合が
完全一致する」テストを置いてある（数値は総和順序のぶん 1 ulp ずれる）。

許される再構築間隔は `K ≤ skin·h/(dt·v_max)`。ダムブレイクでは K ≤ 7.4 なので
**既定は余裕を見て 4**（8 は限界ぎりぎりで実際に警告が出た）。速度を詰めるなら
上げてよいが、変位超過の警告が出ないことを必ず確認すること。

```julia
st = State(backend, X0, V0, p; skin = 0.2, interval = 4, maxnb = 64)
backward!(ws, tape, th, p, backend; seedX, seedV, interval = 4)   # 前進と揃える
```

変位超過と行溢れは GPU 側でフラグに立て、`simulate!` / `backward!` の終わりで
まとめて報告する。毎ステップ見ると同期が入って①の効果が消えるので、事後検出という
割り切りにしてある。**警告が出た設定の勾配は信用しないこと**（実測で 1e-2 ずれた）。
...
| Metal (Float32, dp=0.008) | 1500 | 5333 | -337.95 → **-363.73**（7.6%） | 283 秒 |

どちらも全反復が受理される単調降下。解像度を上げた Metal 側の方が改善幅が
大きく、刻みも上限 0.30 に張り付く（粒子が増えて勾配が滑らかになるため）。
ただし後半の受理判定は上記の非決定性ノイズに埋もれている点に注意。
...
- [x] 近傍リストの実体化（転置・行長固定）+ skin 付き再利用
      （Metal で前進 1.9〜2.2 倍、CPU も 1.8 倍）
- [ ] セル内の粒子順序を決定的にする（GPU の目的関数が 0.29% 揺らぐ根治。
      近傍リストの行をソートする手もあるが挿入ソートが 1.8 倍のコストなので、
      セルリスト側の順序を固定する方が安い）
