...
|---:|---:|---:|---:|
| 216 | 17756 | 8026 | 0.45x |
| 925 | 3047 | 5223 | 1.71x |
| 3750 | 1737 | 5233 | 3.01x |
| 15000 | 669 | 2539 | 3.80x |
| 60000 | 212 | 1028 | 4.85x |

（Metal は実行ごとに ±10% 程度揺れる。上は代表値。）
...
この結果、残る支配項は accel の gather（間接参照）で、次は近傍リストの
実体化（下記）が本命になる。
...
- [ ] 近傍リストの実体化（CSR）+ skin 付き Verlet 化（随伴で 1 ステップ 4 回
      再利用するので償却が効く。文献では本条件が Verlet 有利側）
- [ ] セル幅 h + 5×5 走査（候補 3 割減、上限 1.3 倍）
